...
filter_var = "total_discharge_log"
agg_data %>%
    group_by(gpr_grp) %>%
...
**TEST GLM3
This time, we will pick total discharge in stead of the staffed beds. Then we will compare two models (glm2 vs. glm3) using chisquare test.
```{r}
...
```
Because the anova test yields that glm3 is better, our final model is glm3:
[FINAL MODEL]
