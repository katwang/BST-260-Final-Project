...
5. keep only one record per physician (using NPI, first/last name, graduation year, primary specialty and state and zip just to be extra precise)
```{r}
library(dplyr)
unique_dat <- zip_dat %>% distinct(NPI, Last.Name, First.Name, Graduation.year, Primary.specialty, State, zip, .keep_all = TRUE)
```
