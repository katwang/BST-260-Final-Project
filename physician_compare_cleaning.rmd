...
Import data from `.csv` file, downloaded from the (Medicare Data website)[https://data.medicare.gov/Physician-Compare/Physician-Compare-National-Downloadable-File/].
```{r}
...
unique_dat <- complete_zip %>% distinct(NPI, Last.Name, First.Name, Primary.specialty, City, State, Zip.Code, .keep_all = TRUE)
...
phys <- unique_dat %>% filter(Hospital.affiliation.CCN.1 == "" & Hospital.affiliation.CCN.2 == "" & Hospital.affiliation.CCN.3 == "" & Hospital.affiliation.CCN.4 == "" & is.na(Hospital.affiliation.CCN.5)) # use rgrep to remove all the hospital affiliation CCN and LBN columns later
