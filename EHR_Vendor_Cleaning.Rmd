...
states <- data.frame(state_name = c(state.name,"District of Columbia"), state_abb = c(state.abb, "DC"))
dat_mu <- dat_mu %>% filter(Business_State_Territory %in% states$state_name)

# |   Business_State_Territory == "District Of Columbia"
dat_mu %>% filter(Business_State_Territory == "District of Columba")
# remove zipcodes < 5
...
dat_mu_hosp <- clean_dat %>% filter(Provider_Type == "Hospital") %>% distinct(NPI, CCN, Business_State_Territory, ZIP, Hospital_Type, Program_Type, Provider_Stage_Number, Vendor_Name, EHR_Product_Name, Product_Classification, Product_Setting)
