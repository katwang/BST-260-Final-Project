...
test_mismatch <- dat_mu_zmerge %>% left_join(states, by = c("state" = "state_abb")) %>% mutate(Business_State_Territory = as.character(Business_State_Territory)) %>% filter(state_name != Business_State_Territory) %>% select(state_name, Business_State_Territory, Provider_Type)

#dataset without mismatching zips
test <- dat_mu_zmerge %>%
